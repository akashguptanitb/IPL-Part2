...
const economicalBowlerEachYear = require("./ipl/economicalBowlerEcachYear");
...
const JSON_OUTPUT_FILE_PATH6 = "./data6.json";
...
//sixth problem
    ...
    .then(matches => {
      ...
      .fromFile(DELIVERIES_FILE_PATH)
      .then(deliveries => {
      let result6 = economicalBowlerEachYear(matches,deliveries);
      saveEconomicalBowlerEachYear(result6);
      });
    }); 
}
...
//(sixth Problem)
function saveEconomicalBowlerEachYear(result6) {
  const jsonData6 = {
    economicalBowlerEachYear: result6
  };
  const jsonString6 = JSON.stringify(jsonData6);
  fs.writeFile(JSON_OUTPUT_FILE_PATH6, jsonString6, "utf8", err => {
    if (err) {
      console.error(err);
    }
  });
