...
//Top 10 economical Bowlers(sixth Problem user input)
function myFunction(){
  var inputData = document.getElementById("demo").value;
  
  fetch("/eco?year=" + inputData)
      .then(res => res.json())
      .then(visualizeData6);
  
  
  function visualizeData6(data) {
    visualizeEconomicalBowlerEachYear(data);
    return;
  }
function visualizeEconomicalBowlerEachYear(economicalBowlerEachYear) {
  const seriesData = [];
  for (let year in economicalBowlerEachYear) {
    seriesData.push([year,economicalBowlerEachYear[year]]);
  }

  Highcharts.chart("economical-bowler-each-year", {
    chart: {
      type: "column"
    },
    title: {
